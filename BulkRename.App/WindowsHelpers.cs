...
namespace BulkRename.App
{
    ...
    internal class WindowsHelpers
    {
        ...
        public static void CreateShortCut(string path, string tgtPath)
        {
            var link = (IShellLink)new ShellLink();

            // set shortcut information
            link.SetPath(tgtPath);
            link.SetWorkingDirectory(Path.GetDirectoryName(tgtPath));
            link.SetIconLocation(tgtPath, 0);
